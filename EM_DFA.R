...
Kalman_FS = function(phi_w, Beta, Sigma, Y, w_0, V_0, PX){
...
EM_DSS = function(Y, phi_w, phi_beta = 0.95,
                  lambda0, lambda1, Theta, delta, n0, d0, 
                  NITER = 10000, epsilon = 0.1, PX = FALSE, const_rot = FALSE){
  ...
  while((eps > epsilon) &  (iter < NITER)){
    ...
    KF_result = Kalman_FS(phi_w=0.98, Beta, Sigma, Y, w_0, V_0, PX)
    w = KF_result$w
    w_0 = KF_result$w_0
    V_0 = KF_result$V_0
    ...
    for (j in 1:p){
      ...
      for(t in (1:T)){
        ...
        xrow = t(w_tilde[[t]])
        S[[t]] = solve(diag(D) + xrow %*% t(xrow) * phi[[t]][j])
      }
    ...
    if (PX == TRUE){
      A = list()
      M1 = KF_result$w_0 %*% t(KF_result$w_0) + KF_result$V_0
      M12 = KF_result$w_0 %*% t(KF_result$w[[1]]) + KF_result$Cov[[1]]
      M21 = t(M12)
      ...
      }else{A_rot = matrix(0,k,k)
      for (t in 1:T){
        A_rot = A_rot + 1/T * A[[t]]
      }
      for (t in 1:T){
        A_L_rot = chol(A_rot, pivot=TRUE)
        Beta[[t]] = Beta[[t]] %*% A_L_rot
      }
      }
    }
    
    #for (i in 1:(k-1)){for (j in (i+1):k){Beta0[i,j]=0}}
    #for (t in 1:T){
     #for (i in 1:(k-1)){for (j in (i+1):k){Beta[[t]][i,j]=0}}
    #}
